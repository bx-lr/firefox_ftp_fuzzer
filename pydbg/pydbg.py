...
class pydbg:
    ...
    def addr_to_module (self, address):
        ...
        for module in self.iterate_modules():
            if module.modBaseAddr < address and address < module.modBaseAddr + module.modBaseSize:
                ...
                # so there...
                found = MODULEENTRY32()
                found.szModule = module.szModule
                found.modBaseAddr = module.modBaseAddr
                found.szExePath = module.szExePath

                #@Pedram copy doesn't appear to work on iterators
                #http://www.python.org/dev/peps/pep-0323/
                #so i put this hack there
    ...
    ####################################################################################################################
    def debug_event_loop (self, timeout=None):
        '''
        ...
        while self.debugger_active:
            ...
            if stopTime and time.time() >= stopTime:
                return 1
    ...
    def disasm_around (self, address, num_inst=5):
        ...
        while not complete:
            ...
            while offset < len(slice):
                ...
                instructions.append(("%#x" % addr, inst))
            ...
            for (addr, inst) in instructions:
                if int(addr, 0x10) == address and inst == disassembly:
                    complete = True
                    break
    ...
    ####################################################################################################################
    def short_context (self, context=None):
        ...
        context_dump += "  ESP: %08x" % (context.Esp,)
        return context_dump
    ...
    def load (self, path_to_file, command_line=None, create_new_console=False, show_window=True):
        ...
        if command_line:
            command_line = "\"%s\" %s" % (path_to_file, command_line)
            path_to_file = 0
    ...
    def terminate_process (self, exit_code=0, method="terminateprocess"):
        ...
        # fall back to "terminateprocess".
            ...
            if not kernel32.TerminateProcess(self.h_process, exit_code):
                raise pdx("%s.TerminateProcess(%d)" % (self.h_process, exit_code), True)
